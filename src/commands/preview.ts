...
export default {
	...
	async execute(ctx) {
        ...
        if (args[0] === "new") {
            const variables = await getNewAddonVariables(newAddon, ctx.locale);

            const msg = ctx.dbChat.newAddonMessage;

            const parsedMessage = parseVariables(msg, variables);
        ...
        if (args[0] === "update") {
            const variables = await getUpdatedAddonVariables(updatedAddon, ctx.locale);
